...
ax.plot([0, 1], [0, 1], color="0.55", lw=0.8, ls="--", zorder=1)
ax.scatter(xs, ys, s=13, c="#1a5276", alpha=0.8, linewidths=0, zorder=3)
above = sum(y > x for x, y in zip(xs, ys))
ax.set_xlabel("mean probe score, that task's successes")
ax.set_ylabel("mean score, its failures")
ax.set_xlim(-0.04, 1.04); ax.set_ylim(-0.04, 1.04)
ax.set_xticks([0, 0.5, 1]); ax.set_yticks([0, 0.5, 1])
ax.set_title(f"Round 1, layer 28: {above} of {len(xs)} tasks\nabove the line", fontsize=8, pad=4)
fig.tight_layout()
...
ax.set_title("Instance-level signal peaks mid-network", fontsize=8, pad=4)
fig.tight_layout()
